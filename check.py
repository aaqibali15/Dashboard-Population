...
page = st.sidebar.radio(
    "📌 **Select Page**",
    [
        "🏠 Home", "📈 Population Distribution", "👥 Gender Ratio Analysis", "🏙️ Division-wise Gender Ratio Analysis",
        "📊 Growth Rate Analysis", "🌆 Urban vs Rural Comparison", "🌈 Transgender Population Analysis",
...
if page ==  "🏠 Home":
   ...
   # Calculate total rural, urban, and overall population
    ...
    province_population["TOTAL POPULATION"] = (
    ...
    with col1:
        ...
        rural_population = df.groupby("PROVINCE")["ALL SEXES (RURAL)"].sum().reset_index()
        fig_rural = px.pie(
            rural_population, 
            names="PROVINCE", 
    ...
    with col2:
        ...
        urban_population = df.groupby("PROVINCE")["ALL SEXES (URBAN)"].sum().reset_index()
        fig_urban = px.pie(
            urban_population, 
            names="PROVINCE", 
...
# Population Distribution Page
    ...
    # Select Province
    province = st.selectbox("🌍 Select Province", df["PROVINCE"].unique())

    # Filter dataset based on selected Province
    filtered_df = df[df["PROVINCE"] == province]

    # Calculate total population (Rural + Urban)
    filtered_df["TOTAL POPULATION"] = filtered_df["ALL SEXES (RURAL)"] + filtered_df["ALL SEXES (URBAN)"]
...
# Gender Ratio Analysis
    ...
    # User selects Rural or Urban Population
    area_type = st.selectbox("Select Area Type", ["Rural", "Urban"], index=0)

    # User selects Gender Type
    gender_option = st.selectbox("Select Gender", ["Male", "Female", "Comparison"], index=0)
    ...
    # Select Division (filtered based on selected Province)
    division = st.selectbox("Select Division", province_df["DIVISION"].unique())

    # Filter dataset based on selected Division
    division_df = province_df[province_df["DIVISION"] == division]
...
# Household Size Analysis Page
    ...
    # Select Division (filtered based on Province)
    division = st.selectbox("Select Division", province_df["DIVISION"].unique())
...
# District-wise Insights Page
    ...
    filtered_df = df[(df["PROVINCE"] == province) & (df["DIVISION"] == division) & (df["DISTRICT"] == district)].copy()  # Use .copy() to avoid warnings

    # Creating a new column for total population (Rural + Urban)
    filtered_df["TOTAL POPULATION"] = filtered_df["ALL SEXES (RURAL)"] + filtered_df["ALL SEXES (URBAN)"]
...
# division wise
    ...
    filtered_df = df[(df["PROVINCE"] == province) & (df["DIVISION"] == division)].copy()  # Use .copy() to avoid warnings
...
# Province-wise Insights Page
    ...
    # Select Province
    province = st.selectbox("Select Province", df["PROVINCE"].unique())
    filtered_df = df[df["PROVINCE"] == province].copy()  # Using .copy() to avoid warnings
